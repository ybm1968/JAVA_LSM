...
public class DateTimeCompare {
	...
	public static void main(String[] args) {
		...
		// A.isBefore(B) : A 가 B 보다 이전이면 true 아니면 false
		// A.isEqual(B) : A 가 B 보다 같은면 true 아니면 false
		// A.isAfter(B) : A 가 B 보다 이후이면 true 아니면 false

		if (todayDateTime.isBefore(endDateTime)) {
			System.out.println("교육 진행중");
		} else if (todayDateTime.isAfter(endDateTime)) {
			System.out.println("교육 종료");
		} else {
