 ...
 *  
 *  요소 검색
 *  - element() : head 데이터를 반환
 *  - peek()	: head 데이터를 반환 큐가 비어있으면 null 반환
 *  
 *  요소 삭제
 *  - remove()	: head 데이터를 삭제하고 삭제된 값을 반환(요소가 없을떄도 삭제를 시도해서 NoSuchElementException 예외 발생)
 *  - poll()	: head 데이터를 삭제하고 삭제된 값을 반화 큐가 비어있으면 null 반환(요소가 없으면 null 반환하기 때문에 예외처리를 하지 않아도 된다)
 *  
 *  First In First Out
 */
